...
def main() -> None:
    ...
    with psycopg2.connect(
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT indexname FROM pg_indexes WHERE schemaname=%s AND tablename=%s AND indexdef ILIKE %s",
                ("tripwise", "knowledge_embeddings", "%hnsw%"),
            )
            hnsw = bool(cursor.fetchone())
            print(
                f"Knowledge documents: {documents}; embeddings: {embeddings}; cosine HNSW index: {hnsw}"
            )
