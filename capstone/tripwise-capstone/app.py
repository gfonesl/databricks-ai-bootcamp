...
from config import APP_NAME, MAX_DESTINATIONS, MAX_SEARCH_RESULTS
from embedding import EmbeddingError, embed_texts
from lakebase import LakebaseError, TripwiseRepository
from travel_sources import DestinationContext, OpenMeteoWikimediaClient, TravelSourceError
...
class PipelineContextItem(BaseModel):
    ...
    chunks: list[PipelineChunk] = Field(min_length=1, max_length=100)


class PipelineContextRequest(BaseModel):
    documents: list[PipelineContextItem] = Field(min_length=1, max_length=30)
    ...
    async with mcp_asgi.lifespan(parent_app):
        if os.getenv("TRIPWISE_SKIP_SCHEMA_INIT") != "1":
            try:
                ...
                LOGGER.exception("Tripwise schema unavailable at startup")
        yield


mcp = FastMCP(
    "Tripwise Travel MCP",
    instructions=("Use the Tripwise tools to retrieve destination context and make persistent trip changes. "
...
def mcp_search_destination_context(query: str, trip_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"query": query.strip(), "trip_id": trip_id, "results": results, "source": "Lakebase pgvector"}
...
def mcp_create_trip(display_name: str, title: str, start_date: str, end_date: str, interests: list[str] | None = None, notes: str | None = None) -> dict[str, Any]:
    """Create a persistent Tripwise trip. Dates must use YYYY-MM-DD."""
    try:
        request = TripRequest(display_name=display_name, title=title, start_date=start_date, end_date=end_date, interests=interests or [], notes=notes)
        return repository.create_trip(request.display_name, request.title, request.start_date.isoformat(), request.end_date.isoformat(), request.interests, request.notes)
    except Exception as error:
        return {"error": "trip_create_failed", "message": str(error)}


def mcp_add_itinerary_item(trip_id: int, title: str, scheduled_date: str, destination_id: int | None = None, start_time: str | None = None, notes: str | None = None, is_outdoor: bool = True) -> dict[str, Any]:
    """Add a persistent itinerary item to an existing trip."""
    try:
        request = ItineraryRequest(title=title, scheduled_date=scheduled_date, destination_id=destination_id, start_time=start_time, notes=notes, is_outdoor=is_outdoor)
        return repository.add_itinerary_item(trip_id, request.title, request.scheduled_date.isoformat(), request.destination_id, request.start_time, request.notes, request.is_outdoor)
    except Exception as error:
        return {"error": "itinerary_create_failed", "message": str(error)}


def mcp_reschedule_itinerary_item(itinerary_item_id: int, scheduled_date: str, start_time: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return repository.reschedule_item(itinerary_item_id, request.scheduled_date.isoformat(), request.start_time)
    except Exception as error:
...
def mcp_build_packing_list(trip_id: int) -> dict[str, Any]:
    ...
    try:
        return {"trip_id": trip_id, "items": repository.build_packing_list(trip_id), "source": "Lakebase weather snapshots"}
    except Exception as error:
...
async def normalize_mcp_path(request, call_next):
    """Accept both URI spellings used by Streamable HTTP MCP clients.
...
@app.get("/api/trips")
def list_trips():
    try: return {"trips": repository.list_trips(), "source": "Lakebase tripwise"}
    except Exception as error: raise _http_error(error)


@app.post("/api/trips", status_code=201)
def create_trip(request: TripRequest):
    try: return repository.create_trip(request.display_name, request.title, request.start_date.isoformat(), request.end_date.isoformat(), request.interests, request.notes)
    except Exception as error: raise _http_error(error)
...
def add_destination(trip_id: int, request: DestinationRequest):
    try:
        ...
        row=repository.add_destination(trip_id, ", ".join(part for part in (resolved["name"],resolved.get("country")) if part),resolved["latitude"],resolved["longitude"])
        return row
    except Exception as error: raise _http_error(error)
...
def add_item(trip_id: int, request: ItineraryRequest):
    try: return repository.add_itinerary_item(trip_id,request.title,request.scheduled_date.isoformat(),request.destination_id,request.start_time,request.notes,request.is_outdoor)
    except Exception as error: raise _http_error(error)


@app.patch("/api/itinerary-items/{itinerary_item_id}")
def reschedule_item(itinerary_item_id: int, request: RescheduleRequest):
    try: return repository.reschedule_item(itinerary_item_id,request.scheduled_date.isoformat(),request.start_time)
    except Exception as error: raise _http_error(error)


@app.delete("/api/itinerary-items/{itinerary_item_id}", status_code=204)
def delete_item(itinerary_item_id: int):
    try: repository.delete_item(itinerary_item_id)
    except Exception as error: raise _http_error(error)


@app.post("/api/trips/{trip_id}/packing-list")
def build_packing_list(trip_id: int):
    try: return {"trip_id":trip_id,"items":repository.build_packing_list(trip_id)}
    except Exception as error: raise _http_error(error)
...
def search_context(request: SearchRequest):
    try:
        ...
        return {"query":request.query.strip(),"results":results,"source":"Lakebase pgvector; Open-Meteo + Wikimedia context"}
    except Exception as error: raise _http_error(error)
...
def persist_pipeline_context(request: PipelineContextRequest):
    ...
    try:
        ...
        pending_document_ids = {candidate["document_id"] for candidate in repository.documents_needing_embeddings(limit=200)}
        items_to_embed = [item for item in request.documents if item.document.document_id in pending_document_ids]
        ...
        return {"documents_received": len(documents), "documents_embedded": len(items_to_embed), "chunks_written": chunks_written}
    except Exception as error:
